...
const AddClass = () => {
    ...
    const [feeDetails, setFeeDetails] = useState([{ description: "", amount: "" }]);
    ...
    const handleFeeDetailChange = (index, field, value) => {
        ...
        newFeeDetails[index][field] = field === "amount" ? Number(value) : value;
        setFeeDetails(newFeeDetails);
        const total = newFeeDetails.reduce((acc, item) => acc + (item.amount || 0), 0);
        setTotalFee(total);
    };

    const addFeeDetail = () => {
        setFeeDetails([...feeDetails, { description: "", amount: "" }]);
    };
                        ...
                        <Stack spacing={3}>
                            ...
                            {feeDetails.map((item, index) => (
                                <Stack key={index} direction="row" spacing={2} alignItems="center">
                                    ...
                                    />
                                    {feeDetails.length > 1 && (
                                        <Button variant="outlined" color="error" onClick={() => removeFeeDetail(index)}>
                                            Remove
                                        </Button>
                                    )}
                                </Stack>
                            ))}
                            <Button variant="outlined" onClick={addFeeDetail}>
                                Add Fee Detail
                            </Button>
                            <TextField
                                label="Total Fee"
